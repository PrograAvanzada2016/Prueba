...
public class TileNode
{
  ...
  public TileNode makeNeighbour(int quad, DesarrolloMapa wd)
     ...
     except when that location is invalid according to WorldDisplay. */
  {
    ...
    int y = coord.y;
    if (quad == TilesSprite.NE)
      newNode = ((y%2==0) ? makeNode(x,y-1,wd) : makeNode(x+1,y-1,wd));
    else if (quad == TilesSprite.SE)
       newNode = ((y%2==0) ? makeNode(x,y+1,wd) : makeNode(x+1,y+1,wd));
    else if (quad == TilesSprite.SO)
      newNode = ((y%2==0) ? makeNode(x-1,y+1,wd) : makeNode(x,y+1,wd));
    else if (quad == TilesSprite.NO)
      newNode = ((y%2==0) ? makeNode(x-1,y-1,wd) : makeNode(x,y-1,wd));
    else {
  ...
  private TileNode makeNode(int x, int y, DesarrolloMapa wd)
