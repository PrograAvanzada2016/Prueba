...
{
  private final static String IMAGE_DIR = "Images/"; //Busca las imagenes dentro de la carpeta Images
  ...
  { initLoader(); //Modifica GC respecto al GE (Entorno local gráfico) para que tenga todos los atributos y métodos para trabajar.
    loadImagesFile(fnm);
  }  // end of ImagesLoader()
  ...
  private void initLoader()
  {
    ...
    GraphicsEnvironment ge = GraphicsEnvironment.getLocalGraphicsEnvironment(); //Le brindo a GE el entorno gráfico.
    gc = ge.getDefaultScreenDevice().getDefaultConfiguration();
  }  // end of initLoader()


  private void loadImagesFile(String fnm) //Esta clase lee un TXT que va a contener la forma en la que va a leer la imagen y la ruta de ella.
  /* Formats:
        o <fnm>                     // a single image
        n <fnm*.ext> <number>       // a numbered sequence of images
        s <fnm> <number>            // Tira de imagenes.
        g <name> <fnm> [ <fnm> ]*   // a group of images 

     and blank lines and comment lines.
  */
  { 
    String imsFNm = IMAGE_DIR + fnm; //Nos da la ruta de la imagen (/image/fnm.X).
    System.out.println("Reading file: " + imsFNm); //ESTO SE VA A MOSTRAR EN EL TABLERO DE SERVIDOR!!
    try {
      InputStream in = this.getClass().getResourceAsStream(imsFNm); //InputStream es como un SCANF en C. Habría que ver que es lo que se hace... No se entiende mucho
      BufferedReader br = new BufferedReader( new InputStreamReader(in)); //Lee lo que esta en el buffer.
  ...
  /* format:
        o <fnm>
  */
  { StringTokenizer tokens = new StringTokenizer(line); //StringTokenizer permite separar el string en substrings.
    ...
    if (tokens.countTokens() != 2)
      ...
      loadSingleImage( tokens.nextToken() ); //Aca va a cargar la imagen, pasa como atributo el archivo de la imagen.
    }
  ...
  {
    String name = getPrefix(fnm); //Guardo en name el nombre del archivo SIN la extension.

    if (imagesMap.containsKey(name)) { //Verifico si la imagen ya esta creada en el Hashmap. 
      System.out.println( "Error: " + name + "already used");
      return false;
    }

    BufferedImage bi = loadImage(fnm); //LoadImage SUPONEMOS que carga la imagen.
    if (bi != null) {
      ArrayList imsList = new ArrayList();
      imsList.add(bi);
      imagesMap.put(name, imsList); //Guardo en el HashMap de IMAGENES la imagen con su nombre de archivo.
  ...
  private String getPrefix(String fnm) //Obtiene el nombre del archivo SIN la extension.
  {
    int posn;
  ...
  // --------- load numbered images ------------------------------- ACA VIENEN LOS MÉTODOS PARA CARGAR TIRAS DE IMAGENES. POR EL MOMENTO, NO SE UTILIZAN.
  ...
  */
  ...
  public boolean verificaCargaImagen(String name) //Busca en el Hashmap la imagen.
  {
    ArrayList imsList = (ArrayList) imagesMap.get(name);
    if (imsList == null)
      return false;
  ...
  public int cantImagenes(String name) // cuantas imagenes vamos a almacenar en name??
  {
    ArrayList imsList = (ArrayList) imagesMap.get(name);
    if (imsList == null) {
  ...
  */
   ...
   public BufferedImage loadImage(String fnm) //Retorna un BufferImaged con la carga de la imagen.
   {
     try {
